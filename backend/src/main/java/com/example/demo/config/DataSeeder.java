...
public class DataSeeder implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        // Check BOTH username and email — either one already existing
        // (e.g. from manual testing/registration) must skip the seed,
        // since both columns carry their own unique constraint.
        if (!userRepository.existsByEmail("admin@hirehigh.com")
        ...
        if (!userRepository.existsByEmail("recruiter@hirehigh.com")
                && !userRepository.existsByUsername("recruiter")) {
